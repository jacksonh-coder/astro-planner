...
for name, mag, altitudes, azimuths, times in visible_targets:
    if st.button(f"{name} (등급 {mag})"):
        st.session_state.selected = (name, mag, altitudes, azimuths, times)

if st.session_state.selected:
    name, mag, altitudes, azimuths, times = st.session_state.selected

    fig = go.Figure()
    fig.add_trace(go.Scatter(x=times.datetime, y=altitudes, mode="lines"))
    st.plotly_chart(fig)
